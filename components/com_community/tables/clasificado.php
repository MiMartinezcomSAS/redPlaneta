...
class CTableClasificado extends JTable implements CGeolocationInterface, CTaggable_Item {
    ...
    public function __construct(&$db) {
        parent::__construct('#__community_clasificados', 'id', $db);
    ...
    public function hit($pk = null) {
        ...
        if ($session->get('view-clasificado-' . $this->id, false) == false) {
            parent::hit();
        }
        $session->set('view-clasificado-' . $this->id, true);
    }
    ...
    public function getLargeAvatar() {
        $config = CFactory::getConfig();
        $largeAvatar = $config->getString('imagefolder') . '/avatar/clasificado/clasificado-' . JFile::getName($this->avatar);
    ...
    public function getAvatar() {
        ...
        if ($this->avatar == 'components/com_community/assets/clasificado.png') {
            $this->avatar = '';
            $this->store();
        }

        $avatar = CUrlHelper::avatarURI($this->avatar, 'clasificado.png');
    ...
    public function getThumbAvatar() {
        if ($this->thumb == 'components/com_community/assets/clasificado_thumb.png') {
            $this->thumb = '';
            $this->store();
        }

        $thumb = CUrlHelper::avatarURI($this->thumb, 'clasificado_thumb.png');
    ...
    public function setImage($path, $type = 'thumb', $action = '') {
        ...
        if ($oldFile) {
            ...
            if (!JString::stristr($oldFile, 'clasificado.png') && !JString::stristr($oldFile, 'clasificado_thumb.png') && !$fileInUsed) {
    ...
    public function deleteAllMembers($action = '') {
        ...
        $query = 'DELETE
			       FROM m
				   USING ' . $db->quoteName('#__community_clasificados_members') . ' AS m
				   LEFT JOIN ' . $db->quoteName('#__community_clasificados') . ' AS e on e.id = m.clasificadoid
				   WHERE m.' . $db->quoteName('clasificadoid') . '=' . $db->Quote($this->id) .
                $condition;
    ...
    public function deletePendingMember() {
        ...
        $query = 'DELETE ' . $db->quoteName('member') . ' FROM ' . $db->quoteName('#__community_clasificados_members')
                . ' member INNER JOIN ' . $db->quoteName('#__community_clasificados')
                . ' clasificado ON member.' . $db->quoteName('clasificadoid') . ' = clasificado.' . $db->quoteName('id')
                . ' WHERE member.' . $db->quoteName('status') . '= ' . $db->Quote('0')
                . ' AND clasificado.' . $db->quoteName('enddate') . ' < ' . $db->Quote($now->toSql());
        $db->setQuery($query);
        $db->Query();
    ...
    public function deleteWalls() {
        ...
        $sql = "DELETE
				...
				LEFT JOIN " . $db->quoteName("#__community_activities") . " AS a
					 ON a.id = w.contentid
				WHERE
					 w." . $db->quoteName("type") . " = " . $db->quote("clasificados.wall") . " AND
				     a." . $db->quoteName("cid") . " = " . $this->id . " AND
				     a." . $db->quoteName("app") . " = " . $db->quote("clasificados.wall");
    ...
    public function deleteFeatured() {
        ...
        $sql = "DELETE
				...
				WHERE
				 ...
				 " . $db->quoteName("type") . " = " . $db->quote('clasificados');
    ...
    public function deleteActivity() {
        ...
        $sql = "DELETE
				...
				WHERE
						" . $db->quoteName("cid") . " = " . $db->quote($this->id) . " AND
						" . $db->quoteName("app") . " LIKE " . $db->quote('%clasificados%');
    ...
    public function getAdmins($limit = 0, $randomize = false) {
        ...
        $query = 'SELECT ' . $this->_db->quoteName('memberid') . ' AS id, ' . $this->_db->quoteName('status') . ' AS statusCode FROM '
                . $this->_db->quoteName('#__community_clasificados_members')
                . ' WHERE ' . $this->_db->quoteName('clasificadoid') . ' = ' . $this->_db->Quote($this->id)
        ...
        foreach ($result as $row) {
            $clasificadoAdmin = JTable::getInstance('ClasificadoMembers', 'CTable');
            $clasificadoAdmin->bind($row);
            $data[] = $clasificadoAdmin;
        }
    ...
    public function getAdminsCount() {
        $query = 'SELECT count(a.memberid) FROM '
                . $this->_db->quoteName('#__community_clasificados_members') . ' AS a '
                . ' INNER JOIN ' . $this->_db->quoteName('#__users') . ' AS b '
                . ' WHERE b.' . $this->_db->quoteName('id') . '=a.' . $this->_db->quoteName('memberid')
                . ' AND a.' . $this->_db->quoteName('clasificadoid') . '=' . $this->_db->Quote($this->id)
    ...
    public function getMembers($status, $limit = 0, $randomize = false, $pendingApproval = false) {
        ...
        foreach ($result as $row) {
            ...
            $clasificadoMember->bind($row);
            $clasificadoMember->statusCode = $row->statusCode;
            $data[] = $clasificadoMember;
        }
    ...
    public function getMemberStatus($userid) {
        ...
        $keys = array('clasificadoId' => $this->id, 'memberId' => $userid);
        $member->load($keys);
    ...
    public function isAdmin($userid) {
        ...
        $member = JTable::getInstance('ClasificadoMembers', 'CTable');
        $keys = array('memberId' => $userid, 'clasificadoId' => $this->id);
        $member->load($keys);
    ...
    public function upgradeWallToStream() {
        $this->clasificadoActivitiesMigrate();
    }

    private function clasificadoActivitiesMigrate() {
        $db = JFactory::getDBO();
        ...
        $query = 'SELECT * FROM ' . $db->quoteName('#__community_activities')
                . ' WHERE ' . $db->quoteName('params') . ' LIKE ' . $db->Quote('%clasificados.wall.create%') . ' AND ' . $db->quoteName('params') . ' LIKE ' . $db->Quote('%clasificadoid=' . $this->id . '%');
        ...
        if (!empty($results)) {
            ...
            foreach ($results as &$result) {
                ...
                if (isset($decoded_params['clasificado_url'])) {
                    $group_url = $decoded_params['clasificado_url'];
                ...
                $result['clasificadoid'] = $group_id; // set group id
                ...
                $result['groupid'] = 0;
                $result['app'] = 'clasificados.wall';
                $result['like_type'] = 'clasificados.wall';
                $result['comment_type'] = 'clasificados.wall';
            }
    ...
    public function addWallCount() {
        $query = 'UPDATE ' . $this->_db->quoteName('#__community_clasificados') . ' '
    ...
    public function updateGuestStats() {
        ...
        foreach ($countFields as $key => $value) {
            $query = 'SELECT count(*) FROM ' . $this->_db->quoteName('#__community_clasificados_members') . ' '
                    . ' WHERE '
                    . $this->_db->quoteName('status') . '=' . $this->_db->Quote($value)
                    . ' AND '
                    . $this->_db->quoteName('clasificadoid') . '=' . $this->_db->Quote($this->id);
     ...
     *
     * */
    public function removeGuest($guestId, $clasificadoId) {
        $db = JFactory::getDBO();

        $query = "DELETE FROM " . $db->quoteName("#__community_clasificados_members")
    ...
    public function getLink() {
        ...
        $handler = CClasificadoHelper::getHandler($this);
        return $handler->getFormattedLink('index.php?option=com_community&view=clasificados&task=viewclasificado&clasificadoid=' . $this->id);
    }
    ...
    public function getGuestLink($status = COMMUNITY_EVENT_STATUS_ATTEND) {
        ...
        $handler = CClasificadoHelper::getHandler($this);
        return $handler->getFormattedLink('index.php?option=com_community&view=clasificados&task=viewguest&clasificadoid=' . $this->id . '&type=' . $status);
    }
    ...
    public function getCover() {
        ...
        if (empty($this->cover)) {
            $this->cover = 'components/com_community/templates/default/images/cover/clasificado-default.png';
        }
