...
class CommunityModelClasificados extends JCCModel
	implements CGeolocationSearchInterface, CNotificationsInterface
{
	 ...
	 * Constructor
	 */
	public function CommunityModelClasificados()
	{
		parent::JCCModel();
	...
	public function getTotalGroupEvents($groupId)
	{
		...
		$query = 'SELECT COUNT(*) FROM ' . $db->quoteName('#__community_clasificados') . ' WHERE '
	 ...
	 * @return    array    $result        An array of result.
	 **/
	public function getGroupClasificados($groupId, $limit = 0)
	{
		...
		$query = 'SELECT * FROM ' . $db->quoteName('#__community_clasificados') . ' WHERE '
			. $db->quoteName('type') . '=' . $db->Quote(CEventHelper::GROUP_TYPE) . ' AND '
			. $db->quoteName('contentid') . '=' . $db->Quote($groupId) . ' '
			. ' AND ' . $db->quoteName('enddate') . ' > ' . $db->Quote($pastDate->toSql(true))
			. ' AND ' . $db->quoteName('published') . '=' . $db->Quote('1')
		...
		foreach($result AS $row) {
			$event = JTable::getInstance('Clasificado','CTable');
			$event->bind($row);
			$data[] = $event;
		}
		...
		if (!empty($data)) {
			foreach ($data as $k => $r) {
				$query = "SELECT COUNT(*)
						FROM #__community_clasificados_members a
						JOIN #__users b ON a.memberid=b.id
						WHERE status=1 AND b.block=0 AND clasificadoid=" . $db->Quote($r->id);
				$db->setQuery($query);
	 ...
	 */
	public function getClasificados($categoryId = null, $userId = null, $sorting = null, $search = null, $hideOldEvent = true, $showOnlyOldEvent = false, $pending = null, $advance = null, $type = CEventHelper::ALL_TYPES, $contentid = 0, $limit = null)
	{
		$db = $this->getDBO();
		$join = '';
		$extraSQL = '';
		...
		if (!empty($userId)) {
			$join = 'LEFT JOIN ' . $db->quoteName('#__community_clasificados_members') . ' AS b ON a.' . $db->quoteName('id') . '=b.' . $db->quoteName('clasificadoid');
		...
		$query = 'SELECT DISTINCT a.* FROM '
			. $db->quoteName('#__community_clasificados') . ' AS a '
			. $join
		...
		$query = 'SELECT COUNT(DISTINCT(a.' . $db->quoteName('id') . ')) FROM ' . $db->quoteName('#__community_clasificados') . ' AS a '
			. $join
			. 'WHERE a.' . $db->quoteName('published') . '=' . $db->Quote('1') . ' '
			. $extraSQL;
	 ...
	 * @return type
	 */
	public function getClasificadosIds($userId)
	{
		$db = $this->getDBO();
		$query = 'SELECT DISTINCT a.' . $db->quoteName('id') . ' FROM ' . $db->quoteName('#__community_clasificados') . ' AS a '
			. ' LEFT JOIN ' . $db->quoteName('#__community_clasificados_members') . ' AS b '
			. ' ON a.' . $db->quoteName('id') . '=b.' . $db->quoteName('clasificadoid')
			. ' WHERE '
			. ' ( '
		...
		$db->setQuery($query);
		$clasificadosid = $db->loadColumn();

		if ($db->getErrorNum()) {
			JError::raiseError(500, $db->stderr());
		}

		return $clasificadosid;
	}

	/**
	 * Return the number of groups count for specific user
	 **/
	public function getClasificadosCount($userId)
	{
		...
		$query = 'SELECT COUNT(*) FROM '
			. $db->quoteName('#__community_clasificados_members') . ' AS a '
			. ' INNER JOIN ' . $db->quoteName('#__community_clasificados') . ' AS b '
			. ' ON b.' . $db->quoteName('id') . '=a.' . $db->quoteName('clasificadoid')
	 ...
	 **/
	public function getClasificadosCreationCount($userId)
	{
		...
		$query = 'SELECT COUNT(*) FROM '
			. $db->quoteName('#__community_clasificados') . ' '
	...
	public function getMembersCount($id)
	{
		...
		if (!isset($this->membersCount[$id])) {
			$query = 'SELECT COUNT(*) FROM ' . $db->quoteName('#__community_clasificados_members') . ' '
				. 'WHERE ' . $db->quoteName('clasificadoid') . '=' . $db->Quote($id) . ' '
	 ...
	 */
	public function getCategories($type = CClasificadoHelper::PROFILE_TYPE, $catId = COMMUNITY_ALL_CATEGORIES)
	{
		$db = $this->getDBO();
		$where = '';
		$join = '';
		...
		if ($type != CClasificadoHelper::ALL_TYPES) {
		...
		$query = 'SELECT a.*, COUNT(b.' . $db->quoteName('id') . ') AS count '
			. 'FROM ' . $db->quoteName('#__community_clasificados_category') . ' AS a '
			. ' LEFT OUTER JOIN ' . $db->quoteName('#__community_clasificados') . ' AS b '
	...
	public function getAllCategories()
	{
		...
		$query = 'SELECT * FROM ' . $db->quoteName('#__community_clasificados_category');
		...
		$data = array();
		foreach($result AS $row) {
			$clasificadoCat =	JTable::getInstance('ClasificadoCategory', 'CTable');
			$clasificadoCat->bind($row);
			$data[] = $clasificadoCat;
		}
	...
	function getCategoriesCount()
	{
		...
		$query = "SELECT c.id, c.parent, c.name, count(e.id) AS total, c.description
				  FROM " . $db->quoteName('#__community_clasificados_category') . " AS c
				  LEFT JOIN " . $db->quoteName('#__community_clasificados') . " AS e ON e.catid = c.id
	...
	public function getCategoryName($categoryId)
	{
		...
		$query = 'SELECT ' . $db->quoteName('name') . ' '
			. 'FROM ' . $db->quoteName('#__community_clasificados_category') . ' '
	 ...
	 * Return the number of groups count for specific user
	 **/
	public function getClasificadoLastChild($parent)
	{
		if ($parent == 0)
			return 0;
		...
		$db = $this->getDBO();
		$query = 'SELECT id'
			. ' FROM ' . $db->quoteName('#__community_clasificados')
	 ...
	 */
	public function isClasificadoExist($title, $location, $startdate, $enddate, $id = 0, $parent = 0)
	{
		$db = $this->getDBO();
		...
		$strSQL = 'SELECT count(*) FROM ' . $db->quoteName('#__community_clasificados')
	...
	public function getPending($userId)
	{
		...
		$query = 'SELECT a.*, b.' . $db->quoteName('title') . ', b.' . $db->quoteName('thumb')
			. ' FROM ' . $db->quoteName("#__community_clasificados_members") . ' AS a, '
			. $db->quoteName("#__community_clasificados") . ' AS b'
			. ' WHERE a.' . $db->quoteName('memberid') . '=' . $db->Quote($userId)
			. ' AND a.' . $db->quoteName('clasificadoid') . '=b.' . $db->quoteName('id')
			. ' AND b.' . $db->quoteName('published') . '=' . $db->Quote(1)
	 ...
	 *
	 */
	public function isInvitedMe($invitationId = 0, $userId = 0, $clasificadoId = 0)
	{
		$db = $this->getDBO();

		if ($clasificadoId == 0) {
			$query = "SELECT COUNT(*) FROM "
				. $db->quoteName("#__community_clasificados_members")
			...
			$query = "SELECT * FROM "
				...
				. " AND " . $db->quoteName("clasificadoid") . "=" . $db->Quote($clasificadoId)
				. " AND " . $db->quoteName("status") . "=" . $db->Quote(COMMUNITY_EVENT_STATUS_INVITED)
			...
			$data = array();
			foreach($result AS $row) {
				$member = JTable::getInstance('ClasificadoMembers', 'CTable');
				$member->bind($row);
				$data[] = $member;
			}
	 ...
	 */
	public function getFriendsCount($userid, $clasificadoid)
	{
		$db = $this->getDBO();
		...
		$query = 'SELECT COUNT(DISTINCT(a.' . $db->quoteName('connect_to') . ')) AS id  FROM ' . $db->quoteName('#__community_connection') . ' AS a '
			...
			. ' INNER JOIN ' . $db->quoteName('#__community_clasificados_members') . ' AS c '
			. ' ON a.' . $db->quoteName('connect_from') . '=' . $db->Quote($userid)
			. ' AND a.' . $db->quoteName('connect_to') . '=b.' . $db->quoteName('id')
			. ' AND c.' . $db->quoteName('clasificadoid') . '=' . $db->Quote($clasificadoid) . ' '
	...
	public function getInviteListByName($namePrefix, $userid, $cid, $limitstart = 0, $limit = 8)
	{
		...
		$query = 'SELECT DISTINCT(a.' . $db->quoteName('connect_to') . ') AS id  FROM ' . $db->quoteName('#__community_connection') . ' AS a '
			...
			. ' FROM ' . $db->quoteName('#__community_clasificados_members') . ' AS e  '
			. ' WHERE e.' . $db->quoteName('clasificadoid') . ' = ' . $db->Quote($cid)
	 ...
	 */
	public function getTitle($clasificadoid)
	{
		$db = $this->getDBO();

		$query = 'SELECT ' . $db->quoteName('title') . ' FROM ' . $db->quoteName('#__community_clasificados')
			. " WHERE " . $db->quoteName("id") . "=" . $db->Quote($clasificadoid);
	 ...
	 */
	public function getMonthlyClasificados($month, $year)
	{
		$db = $this->getDBO();
		$mainframe	= JFactory::getApplication();
		$jinput 	= $mainframe->input;
	...
	public function getThumbAvatar($id, $thumb)
	{
		...
		$thumb = CUrlHelper::avatarURI($thumb, 'clasificado_thumb.png');
	 ...
	 */
	public function getClasificadosSearchTotal()
	{
		return $this->total;
	}
	...
	{
		if($user->_cparams->get('notif_clasificados_invite') == 1)
		{
	 ...
	 * @param array    $id
	 **/
	public function deleteExpiredClasificado($id)
	{
		$db = JFactory::getDBO();
	 ...
	 * @return array/object
	 **/
	public function getClasificadoChilds($parent, $advance)
	{
		if ($parent == 0)
			return 0;
		...
		if (isset($advance['clasificadoid']) && $advance['clasificadoid'] > 0) {
			$extraSQL .= ' AND ' . $db->quoteName('id') . '>= ' . $db->Quote($advance['clasificadoid']);
		}
	 ...
	 *
	 **/
	public function getClasificadoChildsCount($parent)
	{
		$db =  $this->getDBO();
	...
	public function updateRecurringImage($path, $type, $parent, $id)
	{
		...
		$query = ' UPDATE ' . $db->quoteName('#__community_clasificados') .
