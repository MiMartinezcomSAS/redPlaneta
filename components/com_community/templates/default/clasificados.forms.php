...
?>
<div class="cLayout cClasificados-Forms">

<form method="post" action="<?php echo CRoute::getURI(); ?>" id="createClasificado" name="createClasificado" class="form-horizontal community-form-validate">
</form>

<form method="post" action="<?php echo CRoute::getURI(); ?>" id="createClasificado" name="createClasificado" class="cForm community-form-validate">

	<script type="text/javascript">

		joms.jQuery(document).ready(function(){

			joms.clasificados.showDesc();

			joms.jQuery("#repeat option[value=" + '<?php echo $clasificado->repeat;?>' + "]").attr("selected", "selected");

			<?php if ($clasificado->id > 0 ) { ?>
		...
		joms.jQuery('#createClasificado').submit(function(clasificado) {

			<?php echo $editor->saveText( 'description' ); ?>

			// show cwindow repeat action for current / future
			<?php if ($clasificado->id > 0 && $clasificado->isRecurring() && $enableRepeat) { ?>
				if (joms.jQuery('#repeataction').val() == '') {
					joms.clasificados.save();
					return false;
				}
			<?php }?>
	...
	<?php if(!$clasificado->id && $clasificadocreatelimit != 0 ) { ?>
		<?php if($clasificadoCreated/$clasificadocreatelimit>=COMMUNITY_SHOW_LIMIT) { ?>
		<div class="hints">
			<?php echo JText::sprintf('COM_COMMUNITY_EVENTS_CREATION_LIMIT_STATUS', $clasificadoCreated, $clasificadocreatelimit ); ?>
		</div>
		<?php } ?>
	<?php } ?>
		...
		<ul class="cFormList cFormHorizontal cResetList">
			...
			<li>
				...
				<div class="form-field">
					<input name="title" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_TITLE_TIPS'); ?>" id="title" type="text" class="required input-block-level jomNameTips" value="<?php echo $this->escape($clasificado->title); ?>" />
					<?php
					if( $helper->hasPrivacy() )
					{
					?>
						<label for="permission-private" class="label-checkbox">
							<input type="checkbox" class="input checkbox" name="permission" id="permission-private" value="1"<?php echo ($clasificado->permission == COMMUNITY_PRIVATE_EVENT ) ? ' checked="checked"' : '';?> />
			...
			<li>
				...
				<div class="form-field">
					<textarea name="summary" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_SUMMARY_TIPS')?>" id="summary" class="input-block-level jomNameTips"><?php echo $this->escape($clasificado->summary);?></textarea>
				</div>
			</li>

			<!-- clasificados description -->
			<li id="clasificado-discription" style="display:none">
				...
				<div class="form-field wysiwyg-field" style="display: none;">
					...
					<?php
					if( !CStringHelper::isHTML($clasificado->description)
						&& $config->get('htmleditor') != 'none'
						&& $config->getBool('allowhtml') )
					{
						$clasificado->description = CStringHelper::nl2br($clasificado->description);
					}
					?>

					<?php echo $editor->displayEditor( 'description',  $clasificado->description , '95%', '150', '10', '20' , false ); ?>
				</div>

				<script type="text/javascript">
					joms.jQuery(window).load(function() {
						if(joms.jQuery(this).width() <= 980) {
							joms.jQuery('.wysiwyg-field').empty().append('<textarea name="description"><?php echo $clasificado->description; ?></textarea>').show();
						} else {
			...
			<!-- clasificados location -->
			<li>
				<label for="location" class="form-label">
					<?php echo JText::_('COM_COMMUNITY_EVENTS_LOCATION'); ?><span class="required-sign"> *</span>
				</label>
				<div class="form-field">
					<input title="<?php echo JText::_('COM_COMMUNITY_EVENTS_LOCATION_TIPS'); ?>" name="location" id="location" type="text" class="required jomNameTips" value="<?php echo $this->escape($clasificado->location); ?>" />
			...
			<!-- clasificados start datetime -->
			<li id="clasificado-start-datetime" class="has-seperator">
					...
					<span class="jomNameTips" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_START_TIME_TIPS'); ?>">
						...
						<script>
							joms.jQuery("#startdate" ).datepicker
								...
								}).datepicker('setDate', "<?php echo $clasificado->startdate;?>"); /* init date when edit clasificado */
						</script>
						<span id="start-time">
			...
			<!-- clasificados end datetime -->
			<li id="clasificado-end-datetime">
				<label class="form-label">
				...
				<div class="form-field">
						...
						<script>
							joms.jQuery("#enddate" ).datepicker
								...
								.datepicker('setDate', "<?php echo $clasificado->enddate;?>"); /* init date when edit clasificado */
				...
				<script type="text/javascript">
					function toggleClasificadoDateTime()
					{
					...
					function toggleClasificadoRepeat()
					{
			...
			<li>
				<div class="form-field">
					<span class="jomNameTips" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_ALL_DAY_TIPS');?>">
						<input class="input checkbox" id="allday" name="allday" type="checkbox" onclick="toggleClasificadoDateTime();" value="1" <?php if($clasificado->allday){ echo 'checked'; } ?> />&nbsp;<?php echo JText::_('COM_COMMUNITY_EVENTS_ALL_DAY'); ?>
					</span>
				</div>
			</li>
			...
			<li>
				...
				<div class="form-field">
					...
					<select name="repeat" id="repeat" onChange="toggleClasificadoRepeat()">
			...
			<?php
			if( $config->get('clasificadoshowtimezone') )
			{
			?>
			<li>
				<label class="form-label">
				...
				<div class="form-field">
					<span class="jomNameTips" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_SET_TIMEZONE'); ?>">
						<select name="offset">
						<?php
						$defaultTimeZone = isset($clasificado->offset)?$clasificado->offset:$systemOffset;
			...
			<li class="has-seperator">
				...
				<div class="form-field">
					<input title="<?php echo JText::_('COM_COMMUNITY_EVENTS_NO_SEAT_DESCRIPTION'); ?>" name="ticket" id="ticket" type="text" size="10" maxlength="5" class="required jomNameTips" value="<?php echo (empty($clasificado->ticket)) ? '0' : $this->escape($clasificado->ticket); ?>" />
					...
					<label for="allowinvite0" class="label-checkbox">
						<input type="checkbox" class="input checkbox" name="allowinvite" id="allowinvite0" value="1"<?php echo ($clasificado->allowinvite ) ? ' checked="checked"' : '';?> />
			...
			<li class="form-action">
				<div class="form-field">
					...
					<?php if(!$clasificado->id): ?>
					<input name="action" type="hidden" value="save" />
					<?php endif;?>
					<input type="hidden" name="clasificadoid" value="<?php echo $clasificado->id;?>" />
					...
					<input type="submit" class="btn btn-primary validateSubmit" value="<?php echo ($clasificado->id) ? JText::_('COM_COMMUNITY_SAVE_BUTTON') : JText::_('COM_COMMUNITY_EVENTS_CREATE_BUTTON');?>" />
				</div>
			</li>
		</ul>
	</form>
...
<script type="text/javascript">
	...
	*/
	joms.jQuery(document).ready(function()
	{
            ...
            toggleClasificadoDateTime();
            toggleClasificadoRepeat();
	});
</script>